...
class Vtk61 < Formula
  ...
  keg_only "VTK 6.1 is deprecated and for visit only."

  # If --with-qt and --with-python, then we automatically use PyQt, too!
  if build.with? "qt" or build.with? "qt52"
  ...
  def install
    ...
    if build.with? "qt" or build.with? "qt52" or build.with? "qt-extern"
      args << "-DVTK_QT_VERSION:STRING=5" if build.with? "qt52"
